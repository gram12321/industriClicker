...
export function calculateSalesOrderBidPremium(input: { customerType: SalesCustomerType; companyPrestige: number; relationship: number; purchasingPower: number; bidMultiplier: number; economyPhase: EconomyPhase; positiveTail?: number; pressurePenalty?: number; minimumPremiumBonus?: number }): number {
  ...
  const purchasingPowerFactor = Math.max(0, input.purchasingPower);
  const bidProfileFactor = Math.max(0, input.bidMultiplier);
...
export class SalesOrders {
  ...
  createDevelopmentOrderForResource(resourceType: ResourceType, quantity: number, globalReferenceUnitPrice: number, maximumOpenOrders: number, currentGameTimeMs: number, companyPrestige: number, quality = 1): SalesOrder | null {
    if (!Number.isInteger(quantity) || quantity < SALES_ORDER_MINIMUM_QUANTITY || quantity > SALES_ORDER_MAXIMUM_QUANTITY || this.offered.length >= maximumOpenOrders || !Number.isFinite(globalReferenceUnitPrice) || globalReferenceUnitPrice <= 0 || !Number.isFinite(quality) || quality <= 0) return null;
