...
export class SalesOrders {
  ...
  private expireOrders(currentGameTimeMs: number, companyPrestige: number): SalesOrder[] { const expired: SalesOrder[] = []; const remaining: SalesOrder[] = []; for (const order of this.offered) { if (order.expiresAtGameTimeMs > currentGameTimeMs) { remaining.push(order); continue; } const expiredOrder = { ...order, status: 'expired' as const, expiredAtGameTimeMs: currentGameTimeMs }; this.completed.unshift(expiredOrder); expired.push(cloneOrder(expiredOrder)); this.applyRelationship(order.customerId, companyPrestige, currentGameTimeMs, -SALES_CUSTOMER_RELATIONSHIP.expiryLoss * (1 + (order.lines.length - 1) * 0.05), 'expired'); } this.offered = remaining; return expired; }
