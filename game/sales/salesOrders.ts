...
export class SalesOrders {
  ...
  private expireOrders(currentGameTimeMs: number, companyPrestige: number): SalesOrder[] { const expired: SalesOrder[] = []; const remaining: SalesOrder[] = []; for (const order of this.offered) { if (order.expiresAtGameTimeMs > currentGameTimeMs) { remaining.push(order); continue; } const expiredOrder = { ...order, status: 'expired' as const, expiredAtGameTimeMs: currentGameTimeMs }; this.completed.unshift(expiredOrder); expired.push(cloneOrder(expiredOrder)); this.applyRelationshipForOrder(expiredOrder, companyPrestige, currentGameTimeMs, 'expired'); } this.offered = remaining; return expired; }
  private applyRelationshipForOrder(order: SalesOrder, companyPrestige: number, currentGameTimeMs: number, outcome: 'fulfilled' | 'rejected' | 'expired'): void { const customer = this.getCustomerCatalogue().find((candidate) => candidate.id === order.customerId); if (!customer) return; const current = this.getCustomerState(order.customerId, currentGameTimeMs, companyPrestige); const adjustment = calculateSalesCustomerRelationshipChange({ outcome, customer, relationship: current.relationship, orderReferenceValue: order.globalReferenceValue }); const next = { ...current, relationship: clamp(current.relationship + adjustment, 0, SALES_CUSTOMER_RELATIONSHIP.maximum), fulfilledOrderCount: current.fulfilledOrderCount + (outcome === 'fulfilled' ? 1 : 0), expiredOrderCount: current.expiredOrderCount + (outcome === 'expired' ? 1 : 0) }; const index = this.customerStates.findIndex((state) => state.customerId === order.customerId); if (index < 0) this.customerStates.push(next); else this.customerStates[index] = next; }
