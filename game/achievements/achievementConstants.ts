...
const ACHIEVEMENT_TIER_PRESTIGE = [
  { amount: 0.1, halfLifeForegroundHours: 1 },
  { amount: 0.4, halfLifeForegroundHours: 5 },
  { amount: 1.5, halfLifeForegroundHours: 10 },
] as const;
...
export function createResourceProductionAchievements(resourceType: ResourceType, resourceName: string, icon: string): AchievementDefinition[] {
  return createTieredAchievements({
    ...
    resourceType,
    thresholds: [25, 250, 1_000, 2_500, 5_000, 10_000, 25_000, 50_000, 100_000, 250_000],
  });
}
...
export const ACHIEVEMENT_DEFINITIONS: readonly AchievementDefinition[] = [
  ...
  ...createTieredAchievements({ seriesId: 'largest_repair', category: 'facilities', name: 'Major Overhaul', description: 'Restore {threshold}% condition in one repair.', icon: 'tools', metric: 'largest-repair', thresholds: [25, 50, 75, 100, 150] }),
  ...
  ...[
    { tier: 1, count: 2, efficiency: 0.5 }, { tier: 2, count: 4, efficiency: 0.75 }, { tier: 3, count: 6, efficiency: 0.9 }, { tier: 4, count: 10, efficiency: 1 }, { tier: 5, count: 15, efficiency: 1.1 },
  ...
  ...createTieredAchievements({ seriesId: 'total_production', category: 'production', name: 'Production Line', description: 'Complete {threshold} total output.', icon: 'package-variant', metric: 'total-produced', thresholds: [25, 250, 1_000] }),
  ...
  ...createTieredAchievements({ seriesId: 'fulfilled_orders', category: 'sales', name: 'Order Closer', description: 'Fulfil {threshold} customer orders.', icon: 'handshake-outline', metric: 'fulfilled-order-count', thresholds: [3, 15, 50] }),
  ...createTieredAchievements({ seriesId: 'fulfilled_quantity', category: 'sales', name: 'Reliable Supplier', description: 'Deliver {threshold} order units.', icon: 'truck-delivery-outline', metric: 'fulfilled-order-quantity', thresholds: [25, 250, 1_000] }),
  ...createTieredAchievements({ seriesId: 'largest_order', category: 'sales', name: 'Big Deal', description: 'Fulfil one order for {threshold} units.', icon: 'briefcase-outline', metric: 'largest-order-quantity', thresholds: [5, 10, 20] }),
  ...createTieredAchievements({ seriesId: 'trusted_customers', category: 'sales', name: 'Trusted Portfolio', description: 'Keep {threshold} customers at 60+ relationship.', icon: 'account-group-outline', metric: 'trusted-customer-count', thresholds: [2, 4, 6] }),
  ...
  ...createTieredAchievements({ seriesId: 'premium_orders', category: 'sales', name: 'Premium Negotiator', description: 'Fulfil {threshold} orders above +10% premium.', icon: 'cash-plus', metric: 'high-premium-order-count', thresholds: [3, 15, 50] }),
  ...
  ...createTieredAchievements({ seriesId: 'company_time', category: 'time', name: 'Active Operator', description: 'Operate for {threshold} foreground minutes.', icon: 'clock-outline', metric: 'foreground-minutes', thresholds: [30, 120, 300] }),
  ...createTieredAchievements({ seriesId: 'company_prestige', category: 'prestige', name: 'Company Standing', description: 'Reach {threshold} company prestige.', icon: 'trophy-outline', metric: 'company-prestige', thresholds: [2, 10, 20] }),
];
