...
/** Quality limit for one facility instance; level 0 starts at Q1. */
export function getFacilityQualityLimit(level: number): number {
  return 1 + calculateDiminishingBonus(
    Math.max(0, Math.floor(level) - 1),
    FACILITY_QUALITY_THEORETICAL_MAXIMUM - 1,
    FACILITY_QUALITY_RESEARCH_RATE,
  );
}
