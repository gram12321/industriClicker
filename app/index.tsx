...
import { calculateCompanyPrestigeSummary, FARM_DEFAULT_SIZE_HECTARES, FacilityType, getMaximumOpenSalesOrders, getNextFirstFacilityTutorialStage, getNextWelcomeTutorialStage, getPreviousFirstFacilityTutorialStage, getPreviousWelcomeTutorialStage, getTutorialProductionPresentation, getWelcomeTutorialStep, isWelcomeTutorialStage, recoverTutorialStage, type TutorialStage, useCompanySessionStore } from '@/game';
...
function GameShell({ companyName }: { companyName: string }) {
  ...
  const isTutorialOpen = isWelcomeTutorialStage(tutorialStage);
  const tutorialStep = getWelcomeTutorialStep(tutorialStage);
  const tutorialPresentation = useMemo(() => getTutorialProductionPresentation(tutorialStage, firstFacilityTutorialRecipeName, firstFacilityRecipeFocusActive), [firstFacilityRecipeFocusActive, firstFacilityTutorialRecipeName, tutorialStage]);
  ...
  const reopenTutorial = () => {
    ...
    if (isWelcomeTutorialStage(recoveredStage)) setActiveView('company');
    else {
