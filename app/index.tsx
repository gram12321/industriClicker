...
import { calculateCompanyPrestigeSummary, getMaximumOpenSalesOrders, getNextFirstFacilityTutorialStage, getPreviousFirstFacilityTutorialStage, getTutorialProductionPresentation, recoverTutorialStage, type FacilityType, type TutorialStage, useCompanySessionStore } from '@/game';
...
function GameShell({ companyName }: { companyName: string }) {
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.safeArea}>
      ...
      <FirstFacilityTutorialDialog focus={firstFacilityTutorialFocus} focusLayout={firstFacilityFocusLayout} facilityType={firstBuiltFacilityType} nextLabel={firstFacilityTutorialStep === 'inventory-transition' ? 'Go to Inventory' : undefined} recipeName={firstFacilityTutorialRecipeName} research={research} onBack={retreatFirstFacilityTutorial} onDismiss={dismissTutorial} onExit={() => { void completeWelcomeTutorial(); }} onNext={advanceFirstFacilityTutorial} step={firstFacilityTutorialStep ?? 'overview'} visible={isFirstFacilityTutorialOpen && activeView === 'production'} />
      <InventoryTutorialDialog onBack={() => { setActiveView('production'); setTutorialStage({ kind: 'first-facility-upgrades' }); }} onExit={() => { void completeWelcomeTutorial(); }} onNext={() => { void completeWelcomeTutorial(); }} visible={isInventoryTutorialOpen && activeView === 'inventory'} />
