...
| `facility.recipeInputQ`, `.recipeInputSourceCost` | Stored | `Facility` | Captured when a recipe cycle consumes inputs; used when that cycle completes | Facility snapshot; quality domain derives InputMaxQ and production adds cycle-caused maintenance to output source cost |
...
| Facility upgrade levels, assigned workers, staff wage per worker per minute, and 0–1 condition | Stored | `Facility` | Upgrade/staffing/wage commands and foreground wear, wage payment, and production | Facility snapshot |
...
| `buildFacility`, `destroyFacility`, `repairFacility`, `setFacilityRecipe`, `setFacilityProductionCycle`, `setFacilityWorkers`, `setFacilityStaffWage`, `setFacilityAutoRepair`, `upgradeFacility` | Facility definition, researched recipes, Repair Technician limit, balance, Construction Materials, and Industrial Machines where applicable | Facilities; Finance; Inventory and resource-flow history where applicable |
...
| Completed production outputs | Recipe outputs, facility output multiplier, captured input source cost, and production-cycle maintenance allocation | Inventory; all-time facility-output resource flow; production achievements; sales targeting |
| Facility per-cycle contribution margin | Current output market value less output source cost | Facility Finance tab; derived only |
| Facility period operating profit / investment-adjusted result | Timestamped Finance-owned facility output source-cost, repair, staff-wage, construction, and upgrade records | Facility Finance tab using a selected Finance report period |
