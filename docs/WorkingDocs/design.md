...
- **Farm footprints:** a Farm is built at a selected immutable 5, 10, 25, or 50 ha footprint, defaulting to 5 ha. Larger Farms bundle the equivalent production batch and staffing into one upgradeable facility; their construction and operating basis scales with footprint.
...
- **Population demand:** paid workers form an aggregate household population that buys the existing resource-domain basket from Local Market stock. The per-minute base basket applies pairwise relative-price substitution within each existing resource group, then household cash determines the continuous purchase scale.
