...
- Customer orders let players exchange available inventory for money. A deterministic customer catalogue generates a variable number of buyers in each domain until that domain reaches 100% market share; each buyer has a market share, purchasing power, and bid profile. Each order locks a reference price, customer bid, premium, lot-sized quantity, and expiry. Relationships change with fulfilment/expiry and decay toward a prestige-derived baseline.
