...
- Customer orders let players exchange available inventory for money. A deterministic customer catalogue generates a variable number of buyers in each home domain until that domain reaches 100% market share; each buyer has a customer type, generated operating domains, market share, purchasing power, and bid profile. Orders are atomic inventory-ready bundles: every resource line is locked and fulfilled together. Customer type controls buyer behaviour rather than a progression ladder, while a skewed maturity distribution makes one line normal and broad compatible bundles an exceptional late-game possibility. Customer/resource selection prefers deeper inventory coverage and stronger existing relationships through bounded weights while retaining production targeting, customer market share, domain frequency, and customer-type frequency. Global premiums are deliberately skewed positive; rare pressure offers can fall below global, while local-market comparison remains independent. The domain target value grows with company prestige and receives a small relationship-volume bonus, keeping offers relevant as the company progresses. New offers are capped by a research-controlled share of current company assets, preventing a single early order from exceeding the company scale. Relationships have no static floor: prestige establishes their baseline; larger fulfilled orders earn more relationship but gains taper at high relationship. Rejection and expiry both scale with trust and order value, but expiry is deliberately harsher at high trust and large late-game order sizes.
- Company prestige represents company standing. It improves customer discovery, bid quality, relationship baselines, and customer-order target value, but does not change production or ordinary market prices.
...
- Research is a primary bottom-navigation view. It presents Capital Grants, Sales Capacity, Order Scope, Sales Targeting, Bid Value, and recipe chains. Each project clearly shows its cost, time, reward, and every unmet condition; active projects are all visible, progress independently within the derived research-capacity limit, and can be cancelled individually for a full refund.

- The ten-tier Local Market Network research expands every local resource market from 1.2× through 8.0× its original depth. Each completed tier starts an independent foreground activation that adds 5% of every resource's original local supply and matching benchmark capacity per minute, so the full tier increment arrives even when the local pool is depleted and later tiers take longer. Overlapping tier activations stack. The separate ten-tier Market Diffusion Network raises only local ↔ regional diffusion from 1.15× through 4.0× normal raw rate; the ordinary equilibrium and source-supply safeguards still apply.
...
- The local market is player-facing; regional and global markets are device-local reservoirs. Resources diffuse between local/regional and regional/global pairs. Fulfilled customer orders deposit goods into the global reservoir. The current deterministic local customer catalogue is intentionally replaceable by a future shared, server-owned registry; only company relationships and order history belong in a save.
- A selected resource in Inventory shows a compact, categorized inventory-flow panel. Its shared 15-second, 1-minute, 15-minute, 1-hour, or all-time foreground-time filter reports Facility output, Production inputs, net Market movement, and conditional Customer orders, Facility spending, and Rewards rows, followed by Net change. The panel appears only in an expanded resource card.
