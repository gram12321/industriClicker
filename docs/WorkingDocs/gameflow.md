...
- Population is currently an aggregate economy view: every assigned facility worker is one population unit, including workers in training. Per-person demand uses the existing Food, Raw Resources, Construction, Manufacturing, and Utilities domain totals and their code-owned resource shares. Facility wages are charged to company Finance and credited to one persisted aggregate household balance during the same foreground step. One minute's available budget is the household balance plus the known per-minute wage rate. The basket has three preference layers: (1) Simulus-style price preference moves demand pairwise within a resource group from the relatively expensive resource to the cheap one, using the moving resource's `resourceElasticity`; (2) only when the price-adjusted basket is unaffordable, baseline preference shifts demand toward lower-baseline fallback goods, leaving higher-baseline goods to be retained when the full basket is affordable; (3) luxury reweights that result further away from high-luxury goods. Each scarcity preference layer preserves the group's total quantity, so an affordable basket has `Affordable = PriceAdj` and an unaffordable basket has a full preferred Affordable basket. Actual is that Affordable basket scaled uniformly down to the available budget, using the slippage-aware Local Market quote; it never exceeds the Affordable amount. Local Market is a cash-free resource reservoir and clearing exchange: it credits company sales when goods enter its stock and the corresponding household purchases debit cash when goods leave, but it persists neither a cash balance nor seller ownership. Individual household accounts and seller ownership remain out of scope.
