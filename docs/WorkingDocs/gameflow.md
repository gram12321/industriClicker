...
- Manual buys/sells trade only with the local market. All inventory and market reservoirs start at Q1; additions and transfers preserve quantity while mixing quality as a quantity-weighted average. Purchases use local market quality, while sales use the sold inventory's quality to derive the unit price and then mix that quality into the local reservoir. A fulfilled customer order adds the delivered inventory and its quality directly to the global reservoir; its reference price, bid, premium, quantity, and reward are locked at offer time.
