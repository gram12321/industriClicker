...
Recipes consume inputs at cycle start; if inputs are absent, the facility stalls without banking work. Their direct-material source cost is captured at that point, so later inventory or market changes cannot alter the in-progress cycle. A completed cycle grants every configured `baseOutput × outputMultiplier`; its captured input cost is allocated across all output units by produced quantity. Maintenance and facility capital investment are excluded. A facility may run one or more researched recipes as an ordered production cycle; when a recipe completes, it starts the next entry and returns to the first after the final entry. A production cycle may repeat a recipe. Each constructed facility is an independent numbered instance, so multiple facilities of one type can run different recipes, cycles, and upgrades. Successful recipe-input consumption and completed outputs both record a categorized resource-flow entry at the foreground step's logical time.
When a facility completes production, Finance records a zero-cash, timestamped performance entry containing its output value at the then-current local-market price and direct input cost. The Facility Finance tab filters those entries through the common Finance report periods. Operating profit is `output value − direct input cost − repair expense`; capital investment is excluded and shown through `investment-adjusted result = operating profit − construction and upgrade investment in the period`.
...
- A repair can restore condition to any selected higher target up to `1`. Cash, Construction Materials, and Industrial Machines each cost `its construction cost × 0.45 × (targetCondition - facilityCondition)`; cash uses land cost. Missing resource inputs are bought automatically from the local market and included in the displayed cash total. The repair dialog previews the selected target's production value, recurring maintenance cost, and net gain per minute.
- Repair Technician research is a five-tier chain. Completed tier `n` allows up to `n` configured facilities to auto-repair. An enabled facility repairs when its condition is at or below its stored threshold, using its stored target condition; the same repair costs, market purchases, maintenance statistics, finance entries, and resource-flow entries as a manual repair. Auto-repair runs during foreground simulation steps only.
...
- A completed Local Market Network tier creates an independent, persisted foreground activation. Its depth increment is the difference between its multiplier and the prior completed tier; every active activation adds `0.05 × localInitialSupply` to each resource's local supply and raises the effective local benchmark multiplier by `0.05` per foreground minute until its own increment is exhausted. Matching fixed baseline additions mean a depleted local pool receives the full planned stock rather than only a percentage of what remains. Activations stack when several tiers are underway, and the effective local benchmark multiplier and activation progress are saved in the market snapshot. This expansion injects local stock independently of regional supply; Market Diffusion Network remains the source-capped local↔regional transfer-rate research.
...
- Manual buys/sells trade only with the local market. All inventory and market reservoirs start at Q1; additions and transfers preserve quantity while mixing quality as a quantity-weighted average. Inventory additions also mix source cost by quantity: a market purchase uses its executed unit price, while output uses the direct-material cost captured when its recipe began. Removals retain the remaining average source cost and clear it only when quantity reaches zero. Purchases use local market quality, while sales use the sold inventory's quality to derive the unit price and then mix that quality into the local reservoir. Each trade uses the average of the relevant unit price immediately before and after its supply change, so larger trades receive normal market slippage and an unchanged-quality buy/sell round trip cannot create cash. A fulfilled customer order adds the delivered inventory and its quality directly to the global reservoir; its reference price, bid, premium, quantity, and reward are locked at offer time.
...
- Current assets are cash plus inventory valued at live local-market prices. Fixed facility book value is the Finance-recorded historical construction and upgrade investment multiplied by `max(0.1, facilityCondition)`; repair costs are maintenance expenses and do not increase that capital. Each facility separately derives its current-price replacement value and market revaluation for information only. Completed research is a capitalized intangible at its configured cost. Total equity is derived as assets less outstanding liabilities.
...
- Completed production records the highest quality ever produced by any facility. The production-quality achievement series checks thresholds Q2, Q5, Q10, Q20, Q35, Q50, Q65, Q80, Q95, and Q99.
...
- Customer acquisition is measured as orders per foreground minute when at least one research-allowed resource has a valid global price and one standard lot fits within the current order-value cap. Rate is `100% base × prestige discovery multiplier × pending-order penalty × company inventory readiness × economy frequency`; prestige discovery ranges from 1% toward 1,000%, company inventory readiness is `0.01 + sqrt(inventory market value / maximum order value)` with no artificial upper cap, and economy acquisition frequency ranges from 33% in a crash to 300% in a boom. The rate is zero only with no offerable resource or when every open-order slot is occupied, and otherwise falls as open orders accumulate. Resource-level inventory coverage remains a selection weight, not a global acquisition multiplier. The sales screen displays the non-base factors and compact estimated wait.
- The deterministic local customer catalogue has Food, Raw Materials, Industrial Inputs, Construction Materials, Electronics, and Utilities buyers. Each domain uses the Winemaker-style customer-generation flow: select a customer type, make skewed market-share draws, take extra draws and keep the smallest for larger candidates, scale by the domain and customer-type share multipliers, then continue until the domain reaches 100%. Local Businesses are strongly compressed toward small shares; enterprise, utility, contractor, and government buyers can absorb larger shares. Customer names use the customer type for their business form and the home domain for their trade vocabulary. Every buyer has market share, purchasing power, and bid profile. Utility orders use high standard lots (Water 500, Electricity 250), so low unit price still produces meaningful order value.
- An order locks one or more offerable resource lines, each with its own customer bid, global reference price, premium, global-supply volume multiplier, lot-sized quantity, and reward. It is fulfilled atomically, but it may request an unstocked resource or more than the current inventory. Quantity per line is `roundUp(line target value × market volume multiplier / bid, standard order lot)`. Standard lots are static commercial units tuned by resource scale and global initial supply. The rolled domain target value is multiplied by `1 + 3 × prestige / (prestige + 250)` (capped below 4×) and by `1 + 0.2 × relationship` (at most 1.2×) before it is distributed across lines, then cannot exceed `max(€100, current company assets × Order Scope fraction)`. Current global supply relative to the resource benchmark applies a bounded final volume multiplier: shortage reaches at most 1.12× and oversupply at most 1.30× by three times benchmark supply. The order-value cap remains authoritative. The base Order Scope fraction is 50%; its five research tiers raise it to 75%, 100%, 150%, 250%, and 400%. Customer type and generated operating domains restrict offerable bundle resources. Bundle line count is a skewed, maturity-weighted draw across compatible resources, using prestige control points 0/1/5/20/60/150/300, normalized 0–1 relationship, market share, and type appetite; there is no fixed line cap beyond compatible resources. Global premiums use a positive-skewed bounded tail, with rare capped pressure offers below global. Fulfilment relationship gain is `domain modifier × 0.10 × global-reference-value / (global-reference-value + €350) × (1 − relationship taper)`, so larger orders earn more but gains flatten as relationship rises. Rejection and expiry use the same relationship-and-value curve, starting at `−0.0001`: rejection approaches `−0.10`, while expiry approaches `−0.20` only at relationship 1 and an effectively uncapped late-game order. The UI multiplies relationship values by 100. Reputation is the prestige recognition less the customer-market-share adjustment; relationship beyond or below that baseline is retained order history. Foreground-time decay moves that history toward Reputation; zero prestige gives no static relationship floor.
- Offer generation selects domain, customer, then resource. Domain weight is `domain frequency × average(production-targeting weight × (0.25 + sqrt(min(64, inventory / standard lot))))` across its offerable resources, so its number of resources or generated customers does not multiply its demand chance. Customer weight within that domain is `sqrt(customer market share) × customer accessibility × customer-type frequency × (1 + relationship)`. The primary resource and every additional bundle resource use `production-targeting weight × (0.25 + sqrt(min(64, inventory / standard lot)))`, keeping unstocked resources possible while favoring inventory coverage and the player's produced-resource research.
- Customer acquisition rolls independently at each foreground simulation step from the displayed orders-per-minute rate. The displayed pipeline is an expected-arrival interval, not a countdown: it fills green through one expected interval, refills red for each additional interval without an order, and resets whenever an order is created. Each acquisition check can create at most one order.
- Capital Grants scale their cost, reward, and foreground duration aggressively; late tiers take hours. Sales Capacity raises the derived open-order maximum from its base of two to 3, 5, 7, 10, or 15. Order Scope raises the maximum value of a newly generated customer order from 50% of company assets through 75%, 100%, 150%, 250%, and 400%, with a €100 practical floor. Sales Targeting gives previously produced resources 2×, 4×, 8×, and 16× offer-selection weight across its first four tiers, then limits offers to those resources when at least one exists. Bid Value raises the customer-order premium from 25% through 30%, 35%, 40%, and 50%; it does not affect ordinary market sales. Capacity blocks only newly created offers; existing offers remain actionable.
- A research project requires all of its gates at start and deducts its full configured cost immediately. The derived research-capacity limit controls how many independent projects may run simultaneously; unrelated chains may run together, but a project whose prerequisites are still active or incomplete remains unavailable. Recipe-unlock research and recipe work-speed follow-ups use three times their base foreground duration. Completion is durable and applies its grant/effect once. Every active project remains separately visible and can be cancelled by project ID, refunding its recorded full paid cost and discarding only that project's partial progress.
- Each resource has an unlimited sequential quality-research chain. Level `n` costs `€100 × 1.12^(n - 1)` and takes `30 seconds × 1.08^(n - 1)` of foreground time. Research and facility upgrade levels use one shared diminishing curve: `Q(progress) = min(99.999999, 1 + 98.999999 × (1 - e^(-ln(99/98) × progress^1.1)))`, with research progress equal to level `n` and facility progress equal to `level - 1`; level 1 research and level 2 facility upgrade are exactly Q2. Lifetime facility output is normalized continuously as `progress = (lifetimeOutput / 100)^0.5` before entering the same curve, so `ProductionMaxQ` improves smoothly with company-wide lifetime production and never reaches Q100. When a recipe cycle consumes inputs, their qualities are averaged by recipe amount. The quality domain returns `InputMaxQ`, `ResearchMaxQ`, `UpgradeMaxQ`, `ProductionMaxQ`, and `OutputQ`; `OutputQ` is the minimum of the four ceilings. Recipes without inputs use an unlimited InputMaxQ. Facility quality upgrades are owned by individual facility instances and their costs align with other facility-upgrade costs. Existing inventory and market quality remains mixed as already held.
...
4. Every active research project advances by the same foreground elapsed milliseconds; background/resume time adds none. Each completed project clears only its own active record and applies its one-time effect atomically.
...
No save compatibility layer exists: a snapshot missing a required current field is discarded. The current quality-aware snapshot requires the facility input Q, quality-upgrade level, and resource-flow highest-output-quality history. Changes to persisted gameplay units intentionally invalidate former saves through the existing structural validation.
