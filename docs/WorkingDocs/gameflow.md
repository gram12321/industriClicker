...
- A recipe consumes required inputs at cycle start or stalls without banking work. Optional inputs are configured per facility and recipe; a checked optional input is autobought with the production cycle and consumed only when its full amount is available. Its deterministic effects are captured at cycle start with input quality and source cost, then retained through partial progress and save/load. Optional effects currently support a post-ceiling quality boost, an output multiplier, and a multiplier (`0 < m <= 1`) on other required inputs.
