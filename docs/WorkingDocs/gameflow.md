...
- Population is currently an aggregate economy view: every assigned facility worker is one population unit, including workers in training. Per-person demand uses the existing Food, Raw Resources, Construction, Manufacturing, and Utilities domain totals and their code-owned resource shares. Facility wages are charged to company Finance and credited to one persisted aggregate household balance during the same foreground step. One minute's available budget is the household balance plus the known per-minute wage rate. The basket has three preference layers: (1) Simulus-style price preference moves demand pairwise within a resource group from the relatively expensive resource to the cheap one, using the moving resource's `resourceElasticity`; (2) baseline preference reweights the price basket within each resource group; (3) luxury reweights that result further away from high-luxury goods. Each preference layer preserves the group's total quantity, so their result is the full adjusted basket. The actual basket is that adjusted basket scaled uniformly down to the available budget, using the slippage-aware Local Market quote; it never exceeds the adjusted amount. Local Market is a cash-free resource reservoir and clearing exchange: it credits company sales when goods enter its stock and the corresponding household purchases debit cash when goods leave, but it persists neither a cash balance nor seller ownership. Individual household accounts and seller ownership remain out of scope.
