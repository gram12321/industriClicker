...
- Population is currently an aggregate economy view: every assigned facility worker is one population unit, including workers in training. Per-person base demand is a code-owned per-game-minute virtual basket grouped by the current resource domains. Each domain owns a provisional total and its direct resources are relative shares of that total. Food is diversified across basic and finished goods, Water and Electricity have direct utility demand, Wool has a deliberately small direct household demand, and the small raw/construction/manufacturing entries are implementation probes that can be rebalanced later. Facility wages are charged to company Finance and credited to one persisted aggregate household balance during the same foreground step. After wages, each foreground step prorates the basket, purchases its affordable and available share from Local Market stock, and debits the household balance at the existing slippage-aware local execution price. Local Market is a resource reservoir and clearing exchange, so it does not hold a cash balance. When cash or stock is short, the whole basket is fulfilled proportionally. The population ledger retains fulfilled purchases by resource for the current foreground game minute; the People view shows this actual amount beside the target basket, ready for later substitution effects. Individual household accounts and seller ownership remain out of scope.
