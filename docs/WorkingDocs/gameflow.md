...
- A recipe consumes required inputs at cycle start or stalls without banking work. Optional inputs are configured per facility and recipe; a checked optional input is autobought with the production cycle and consumed only when its full amount is available. Optional inputs sharing an alternative group are mutually exclusive, so at most one is consumed per cycle. Its deterministic effects are captured at cycle start with input quality and source cost, then retained through partial progress and save/load. Optional effects currently support a post-ceiling quality boost, an output multiplier, and a multiplier (`0 < m <= 1`) on other required inputs. Recipe outputs may also apply a positive quality multiplier after the normal ceiling and additive bonuses, allowing sub-Q1 outputs such as synthetic Leather.
- A no-input recipe may set required work on each output. Those outputs receive the facility's effective work concurrently, retain their own persisted progress, and complete with their own output-quality, maintenance allocation, flow, and Finance entry; ordinary recipes still complete all outputs together.
...
- Construction consumes land euros, Construction Materials, and Industrial Machines. Construction-input requirements are measured in Q1-equivalent units: a quantity of quality Q contributes `quantity × Q` units, so Q2 input supplies twice the construction credit of Q1. This applies generically to every resource used as a construction, upgrade, or repair input; required resource types remain separate. Farms and Forestry facilities are built at an immutable 5, 10, 25, or 50 ha footprint; 5 ha is the baseline and larger footprints scale construction, upgrade, repair, staffing, recipe inputs/outputs, and required work. The scaled work capacity includes the facility's baseline work contribution, preserving the production rate of the equivalent number of baseline land facilities. Missing construction/upgrade/repair inputs may be bought from the local market only when the full cash requirement is affordable.
- Chemical Plant consumer products are Household Cleaning Products, Paint & Home Coatings, and Garden Supplies. Paint & Home Coatings is also consumed by the Construction Factory when producing Construction Materials.
- Display Panels are an intermediate-only Electronics Factory output. Electronics Factory Personal Electronics consumes 0.5 panels per unit, while Assembly Plant Household Appliances consumes 0.1 panels per unit and also requires Advanced Components; this keeps display demand concentrated in Personal Electronics while leaving panels reusable for future display-based products.
...
- Required workers: `baseWorkers + speedLevel + outputLevel + ceil(baseWorkers × 1.15^(speedLevel + outputLevel) - baseWorkers)`. Available workers are assigned workers not currently training; each available worker contributes direct work, while the staffing-efficiency multiplier applies the existing understaffing penalty or diminishing overstaffing bonus. Worker staffing efficiency is `0.01 + 0.99 × ratio^1.6` at/below target and `1 + 0.25 × (1 - e^(-0.7 × (ratio - 1)))` above it, multiplied by wage efficiency and Staff Quality's work multiplier. The wage reference is the dynamic neutral target `€1 × Staff Quality × economyPhaseMultiplier`; distance above or below that target uses a normalized exponential curve, so farther wages create stronger Staff Quality pressure while the result remains capped.
- Staff wages are charged before each foreground production step. When the charge cannot be paid, production, staff training time, and wage-driven Staff Quality/experience progression pause; unpaid training retains its remaining duration. Repair may overlap staffing and training; staffing and training remain mutually exclusive. Hiring and firing use timed wage-based costs, and firing removes the fired workers' proportional share of pooled facility knowledge. The Staff Quality trend is a decaying net direction with an approximately one-foreground-minute memory, so staffing, wage, training, and production-experience effects are shown together instead of exposing only the last tick's mutation.
...
- Wage-only Staff Quality pressure is reported as the current Q change per foreground minute after the quality curve. The neutral target is `€1 × Staff Quality × economyPhaseMultiplier`, with phase multipliers Crash 0.70, Recession 0.85, Stable 1.00, Expansion 1.15, and Boom 1.30. Training and production experience are separate effects.
...
- Effective work is `(baseWork + availableWorkers × 0.1 × stepMs/60,000 × staffingEfficiency) × conditionEfficiency × speedMultiplier × recipeResearchMultiplier`.
- For a scaled Farm, effective work is `(baseWork × footprintMultiplier + availableWorkers × 0.1 × stepMs/60,000 × staffingEfficiency) × conditionEfficiency × speedMultiplier × recipeResearchMultiplier`; recipe condition loss remains per scaled batch rather than multiplying the 0–1 condition loss, because the repair basis already scales with the footprint.
...
- Population is currently an aggregate economy view: every assigned facility worker is one population unit, including workers in training. Per-person demand uses the existing Food, Raw Resources, Construction, Manufacturing, and Utilities domain totals and their code-owned resource shares. Facility wages are charged to company Finance and credited to one persisted aggregate household balance during the same foreground step. One minute's available budget is the household balance plus the known per-minute wage rate. The basket has four preference layers: (1) Simulus-style price preference moves demand pairwise within a resource group from the relatively expensive resource to the cheap one, using the geometric mean of the leaving and entering resources' `resourceElasticity`; (2) low-elasticity horizontal price preference shifts domain spending between relatively expensive and cheap domains, using combined domain benchmark costs and domain `essential` resistance on the losing side; (3) only when the price-adjusted basket is unaffordable, baseline preference shifts demand toward lower-baseline fallback goods, leaving higher-baseline goods to be retained when the full basket is affordable; (4) luxury reweights that result further away from high-luxury goods. Each scarcity preference layer preserves the group's total quantity, while horizontal substitution preserves total current-price domain spending. An affordable basket has `Affordable = PriceAdj` and an unaffordable basket has a full preferred Affordable basket. Actual is that Affordable basket scaled uniformly down to the available budget, using the slippage-aware Local Market quote; it never exceeds the Affordable amount. Local Market is a cash-free resource reservoir and clearing exchange: it credits company sales when goods enter its stock and the corresponding household purchases debit cash when goods leave, but it persists neither a cash balance nor seller ownership. Individual household accounts and seller ownership remain out of scope.
...
- Adjacent pools diffuse every five foreground seconds from the higher `fillRatio = supply / benchmarkSupply` to the lower. Raw request is `rateBaseSupply / divisor × saturationGap × (1 + saturationGap)^curvature × logisticsMultiplier × valueDensityMultiplier × localRegionalResearchMultiplier`, where `saturationGap = max(lowerFillRatio / higherFillRatio, higherFillRatio / lowerFillRatio) - 1`; elapsed-time, benchmark-capacity equilibrium, and source-supply caps apply. Diffusion ignores quality; quality continues to affect local trade prices and mixes into receiving stock. Diffusion research changes only local↔regional raw rate and uses the configured 1.15×–4.00× tiers.
...
- Finance stores signed typed transactions, nested details, logical timestamps, loans, lender portfolios/searches, economy phase, and payment history. Repetitive foreground wages, automatic market trades, and facility-production performance are coalesced into one exact-total ledger entry per source/facility-or-resource/minute bucket, retaining their occurrence count for cash-flow presentation. Finite reports assign coalesced entries to their minute-bucket start so a partial neighboring bucket is not counted twice; reports use foreground windows of 1 minute, 15 minutes, 1 hour, 10 hours, 24 hours, or all time.
...
- Quality research is unlimited per resource. Level `n` costs `€100 × 1.12^(n - 1)` and takes `30 seconds × 1.08^(n - 1)`. Base output Q is `min(InputMaxQ, ResearchMaxQ, UpgradeMaxQ, ProductionMaxQ, StaffMaxQ)`; recipe-specific bonuses are then added. Bake Premium Cake produces Cake and adds Q1 (the player-facing +100% quality bonus). Research/facility levels use `Q(progress) = min(99.999999, 1 + 98.999999 × (1 - e^(-ln(99/98) × progress^1.1)))`, and lifetime output uses `progress = (lifetimeOutput/100)^0.5`. The base ceiling never reaches 100.
...
3. Each step applies passive facility wear, wages and population Local Market purchases, market-network activation, autotrade, production, auto-repair, sales acquisition/expiry/relationships, market diffusion, and active research.
...
| Event | Behavior |
|---|---|
| Normal change | Batch the current company snapshot for up to five seconds; ordinary saves serialize current state without advancing foreground simulation, and only one save may run at a time. |
