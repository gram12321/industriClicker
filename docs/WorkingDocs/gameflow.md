...
- Each resource has an unlimited sequential quality-research chain. Level `n` costs `€100 × 1.5^(n - 1)` and takes `30 seconds × 1.5^(n - 1)` of foreground time. Its research quality is `Q = 1 + 99 × (1 - e^(-ln(99/98) × n))`: level 1 is exactly Q2, later increments diminish, and the curve approaches Q100 without an explicit runtime cap. Lifetime facility output of the produced resource supplies a fourth quality limit. Its exponential intervals unlock Q2/Q5/Q10/Q20/Q40 at 100/4,000/10,000/30,000/80,000 output, then grow by 10% per quality level through Q99. When a recipe cycle consumes inputs, their qualities are averaged by the recipe amounts; each output quality is `min(resource research quality, weighted input quality + 1, facility quality limit, lifetime-production quality limit)`. Recipes without inputs use the resource research, facility, and lifetime-production limits. Facility quality upgrades are owned by individual facility instances; level 1 is the baseline Q1 limit, the first purchase creates level 2 and a Q2 limit, and later levels use the same diminishing-return progression. Upgrade costs align with other facility upgrades. A future dedicated research resource may be added to those costs, such as Research Service or Computer Resource. Existing inventory and market quality remains mixed as already held.
