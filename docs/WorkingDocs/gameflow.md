...
- Each foreground minute may acquire one customer order when a meaningful inventory lot is available within the current order-value cap. Chance is `100% base × (65% + prestige discovery progress) × pending-order penalty × economy frequency`; it is zero without an eligible lot or when every open-order slot is occupied, and otherwise falls as open orders accumulate. The sales screen displays the non-base factors and compact estimated wait.
...
- Eligible customer/resource pairs are weighted by `production-targeting weight × sqrt(min(64, inventory / standard lot)) × customer market share × domain frequency × customer-type frequency × (1 + relationship)`. This prefers deeper stock and established customer affinity while keeping both effects bounded alongside the existing targeting and catalogue factors. Additional bundle resources also use the bounded stock-coverage factor.
