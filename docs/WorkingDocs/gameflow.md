...
- An order locks a customer bid, reference price, premium, quantity, and reward. Quantity is `roundUp(target value / bid, standard order lot)`. Full fulfilment raises relationship and creates a decaying sales-prestige event; rejection has no penalty; expiry reduces relationship. Relationships decay toward a prestige- and market-share-derived baseline during foreground time.
