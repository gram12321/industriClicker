...
- Population is currently an aggregate economy view: every assigned facility worker is one population unit, including workers in training. Per-person demand uses the existing Food, Raw Resources, Construction, Manufacturing, and Utilities domain totals and their code-owned resource shares. Facility wages are charged to company Finance and credited to one persisted aggregate household balance during the same foreground step. The consumer selects one minute of purchases from the known per-minute wage rate or existing household balance, whichever is larger, then buys that basket proportionally during each foreground step. The base basket is adjusted by Simulus-style pairwise substitution: each unique nonzero resource pair within an existing resource group compares `currentPriceA / currentPriceB` with `referencePriceA / referencePriceB`, where a reference price is `localBenchmarkSupply / localInitialSupply`. A capped transfer moves from the relatively expensive resource to the relatively cheap resource; outgoing transfers are normalized so no resource loses more than its base amount. The adjusted basket is scaled to the available per-minute budget. The resulting stock-capped purchases use the existing slippage-aware Local Market execution quote. Local Market is a cash-free resource reservoir and clearing exchange: it credits company sales when goods enter its stock and the corresponding household purchases debit cash when goods leave, but it persists neither a cash balance nor seller ownership. Individual household accounts and seller ownership remain out of scope.
