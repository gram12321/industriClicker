...
- Each foreground minute may acquire one customer order when a meaningful inventory lot is available. Chance is base acquisition × prestige discovery × pending-order penalty × economy frequency; it is zero without an eligible lot and falls as open orders accumulate.
...
- An order locks one or more inventory-ready resource lines, each with its own customer bid, global reference price, premium, lot-sized quantity, and reward. It is fulfilled atomically. Quantity per line is `roundUp(line target value / bid, standard order lot)`. The rolled domain target value is multiplied by `1 + 3 × prestige / (prestige + 250)` (capped below 4×) and by `1 + 0.2 × relationship / 100` (at most 1.2×) before it is distributed across lines. Customer type and generated operating domains restrict eligible resources. Bundle line count is a skewed, maturity-weighted draw across compatible in-stock resources, using prestige control points 0/1/5/20/60/150/300, 0-100 relationship, market share, and type appetite; there is no fixed line cap beyond compatible resources. Global premiums use a positive-skewed bounded tail, with rare capped pressure offers below global. Full fulfilment raises relationship and creates a decaying sales-prestige event; rejection has no penalty; expiry reduces relationship. Relationships decay toward a prestige- and market-share-derived baseline during foreground time.
