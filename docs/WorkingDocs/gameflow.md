...
- Foreground minute completion creates price-locked sales offers, then source-capped diffusion balances every resource first between local/regional and then regional/global reservoirs. Autosell runs independently at each resource's selected 5-second, 20-second, 1-minute, 3-minute, or 10-minute foreground interval; its configured maximum remains a per-minute rate and is scaled to the interval. Offline time does none of these.
...
- Lender searches accept lender-type, amount, term, and offer-count criteria, charge a deterministic search fee up front, and become a foreground-time activity. Offer count and narrowed criteria increase both fee and work; matching offers appear only on completion. Accepted loans add proceeds and an origination fee as financing cash flow.
