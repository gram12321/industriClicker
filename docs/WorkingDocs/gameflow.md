...
- Population is currently a read-only derived view: every assigned facility worker is one population unit, including workers in training. Per-person base demand is a code-owned per-game-minute catalogue (currently one interim Food unit split 80% Bread and 20% Grain) and is multiplied by population for display only. The view also estimates the per-minute local purchase cost from current prices; it does not yet consume market supply, settle purchases, create household balances, or persist population state.
