...
- Each resource has an unlimited sequential quality-research chain. Level `n` costs `€100 × 1.5^(n - 1)` and takes `30 seconds × 1.5^(n - 1)` of foreground time. Its output quality is `Q = 1 + 99 × (1 - e^(-ln(99/98) × n))`: level 1 is exactly Q2, later increments diminish, and Q100 is never reached. A completed level applies immediately to every future facility output of that resource; existing inventory and market quality remains mixed as already held.
