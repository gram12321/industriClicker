...
- Manual buys/sells trade only with the local market. Each trade uses the average of the local unit price immediately before and after its supply change, so larger trades receive normal market slippage and an unchanged-quality buy/sell round trip cannot create cash. A fulfilled customer order adds the delivered inventory and its quality directly to the global reservoir; its reference price, bid, premium, quantity, and reward are locked at offer time.
