...
| Resource catalogue | Code-owned resource identity, display name, benchmark, supply, and market metadata. Exact values live in `game/resources/resourceConstants.ts`. |
...
| Source cost | Historical euro cost per inventory unit: executed market price for purchases, direct input cost for facility output. |
| Direct material cost | Source cost of inputs consumed by one recipe cycle; it excludes maintenance and capital investment. |
...
| Local / regional / global market | Player-facing local reservoir, device-local regional reservoir, and device-local global reservoir. |
| Local market depth | Research-driven foreground activation that adds local supply and capacity. |
| Diffusion rate / interval | Research multiplier for local↔regional requests / five-second adjacent-market cadence. |
