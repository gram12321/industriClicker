...
| Customer order | A customer-specific, atomic bundle of one or more inventory-ready resource lines, with locked global reference prices, bids, premiums, lot-sized quantities, expiry, and full-fulfilment requirement. The domain target value scales with company prestige and receives a modest relationship volume bonus. An order is not a future contract. |
