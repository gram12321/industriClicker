...
| Population / household balance | Each assigned facility worker is one aggregate population unit. Successfully paid facility wages credit one persisted household balance; foreground purchases debit it and remove fulfilled goods from Local Market stock. Local Market is a cash-free clearing exchange: company sale proceeds and later household purchase debits are the two sides of its resource clearing, while the market itself stores neither cash nor seller ownership. |
| Population consumption | One code-owned per-minute catalogue based on the existing Food, Raw Resources, Construction, Manufacturing, and Utilities domain totals. Each resource has a base amount plus 0–1 baseline-preference, luxury, and resource-elasticity values. |
| Consumer substitution | Within an existing resource group, every unique nonzero resource pair compares its live-price ratio to the initial local reference-price ratio. Demand transfers from the relatively expensive resource to the relatively cheap one; transfers are capped and normalized so no resource loses more than its base amount. |
