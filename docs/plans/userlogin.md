...
Profile preferences
  -> owns the selected UI theme and future player-level preferences
```
...
|---|---|
| `game/company/companyDatabase.ts` | Own company-keyed snapshots plus local player, company, session, preference, and tutorial SQLite CRUD. The previous one-row save is deliberately discarded; do not add a compatibility layer. |
...
| `app/index.tsx` | Replace the inert logout action with local-session logout; route Profile, Settings, Leaderboard, and documentation dialogs through the new surfaces. |
| `ProfileDashboard.tsx` and `CompanyDashboard.tsx` | Replace the placeholders with a small profile/company summary and company-management entry points. |
| `theme.ts` | Keep visual tokens here; persist only the selected named theme preference, not duplicate colour data per company. |
...
| Profile preferences | Player | Profile ID, selected theme ID. |
...
- Validate public create/update inputs before persistence: trimmed non-empty names, a bounded visible length, valid owner/company relation, known theme ID, known starting-condition ID, and valid snapshots.
- Do not expose raw SQLite rows beyond their owning `*Database.ts` module. Domain services return typed local profile, company, preference, and session values.
...
- Include the theme choice in this first setup surface, defaulted safely; it is saved to the selected local profile, not the new company.
...
| Settings | Theme picker, tutorial replay, local-data explanation, and logout. | Notifications, accessibility preferences, backup/sync controls. |
...
5. Wire Profile, Settings, real local logout, theme preference, tutorial replay, and reset-this-company behaviour.
