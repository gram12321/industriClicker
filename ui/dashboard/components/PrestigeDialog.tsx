    ...
    <Dialog.Content style={styles.content}>
      ...
      <ScrollView contentContainerStyle={styles.eventList} nestedScrollEnabled showsVerticalScrollIndicator style={[styles.eventViewport, { maxHeight: Math.max(160, height - 440) }]}>
        {events.length === 0 ? <Text style={styles.emptyText}>No matching prestige events yet.</Text> : events.map((event) => {
          ...
          return <Pressable accessibilityLabel={`${isSelected ? 'Hide' : 'Show'} prestige details for ${EVENT_LABELS[event.type]}`} key={event.id} onPress={() => setSelectedEventId(isSelected ? null : event.id)}><Surface elevation={0} style={[styles.eventRow, isSelected && styles.selectedEventRow]}><View style={styles.eventText}><Text variant="bodyLarge">{EVENT_LABELS[event.type]}</Text><Text style={styles.eventDescription}>{event.description}</Text><Text style={styles.eventDecay}>{event.decayHalfLifeYears === null ? 'Permanent' : `Fading: halves every ${formatNumber(event.decayHalfLifeYears, { smartDecimals: true })} prestige years`}</Text><Text style={styles.tapHint}>{isSelected ? 'Tap to hide details' : 'Tap to show details'}</Text></View><Text style={event.currentAmount < 0 ? styles.penaltyAmount : styles.eventAmount}>{formatSigned(event.currentAmount, { smartDecimals: true })}</Text><Icon source={isSelected ? 'chevron-up' : 'chevron-down'} size={22} color={colors.muted} /></Surface></Pressable>;
        })}
      </ScrollView>
    </Dialog.Content>
