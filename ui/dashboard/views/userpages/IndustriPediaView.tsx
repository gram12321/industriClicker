...
export function IndustriPediaView({ companyPrestige, currentGameTimeMs, economyPhase, initialCustomerId, initialSection = 'resources', market, salesOrders }: { companyPrestige: number; currentGameTimeMs: number; economyPhase: EconomyPhase; initialCustomerId?: string | null; initialSection?: IndustriPediaSection; market: Market; salesOrders: SalesOrders }) {
  ...
  return <>
    ...
    {activeSection === 'customers' && <CustomersSection companyPrestige={companyPrestige} currentGameTimeMs={currentGameTimeMs} initialCustomerId={initialCustomerId} salesOrders={salesOrders} />}
  </>;
}

export type IndustriPediaSection = 'resources' | 'facilities' | 'recipes' | 'market-flow' | 'finance' | 'economy' | 'loans' | 'prestige' | 'achievements' | 'customer-domains' | 'customer-types' | 'customers';
...
function CustomersSection({ companyPrestige, currentGameTimeMs, initialCustomerId, salesOrders }: { companyPrestige: number; currentGameTimeMs: number; initialCustomerId?: string | null; salesOrders: SalesOrders }) {
  const catalogue = salesOrders.getCustomerCatalogue();
  const initialCustomer = catalogue.find((customer) => customer.id === initialCustomerId);
  const [selectedDomain, setSelectedDomain] = useState<SalesCustomerDomain | 'all'>(initialCustomer?.domain ?? 'all');
  const [selectedType, setSelectedType] = useState<SalesCustomerType | 'all'>(initialCustomer?.customerType ?? 'all');
  const [knownOnly, setKnownOnly] = useState(false);
  const [sortKey, setSortKey] = useState<CustomerSortKey>('marketShare');
  const [sortDescending, setSortDescending] = useState(true);
  const [sortMenuVisible, setSortMenuVisible] = useState(false);
  const [typeMenuVisible, setTypeMenuVisible] = useState(false);
  const [expandedCustomerId, setExpandedCustomerId] = useState<string | null>(initialCustomerId ?? null);
  ...
  return <>
    ...
    {customers.map(({ customer, state, rejected, relationship }) => { const isExpanded = expandedCustomerId === customer.id; const relationshipDetails = calculateSalesCustomerRelationshipDetails(customer, companyPrestige); return <Card key={customer.id} mode="contained" style={[styles.featureCard, customer.id === initialCustomerId && localStyles.highlightedCustomer]}><Pressable accessibilityLabel={`${isExpanded ? 'Hide' : 'Show'} details for ${customer.name}`} accessibilityRole="button" accessibilityState={{ expanded: isExpanded }} onPress={() => setExpandedCustomerId((current) => current === customer.id ? null : customer.id)}><Card.Content style={styles.cardContent}><View style={localStyles.customerHeader}><View style={localStyles.customerTitle}><View style={localStyles.customerTypeHeading}><MaterialCommunityIcons accessibilityLabel={SALES_CUSTOMER_TYPE_PROFILES[customer.customerType].label} color={colors.primary} name={SALES_CUSTOMER_TYPE_ICONS[customer.customerType] as never} size={18} /><Text variant="titleMedium">{customer.name}</Text></View><View style={localStyles.customerSubheader}><MaterialCommunityIcons accessibilityLabel={SALES_CUSTOMER_DOMAIN_PROFILES[customer.domain].label} color={colors.muted} name={SALES_CUSTOMER_DOMAIN_ICONS[customer.domain] as never} size={14} /><Text style={styles.cardDescription}>{SALES_CUSTOMER_DOMAIN_PROFILES[customer.domain].label}</Text><MaterialCommunityIcons accessibilityLabel={SALES_CUSTOMER_TYPE_PROFILES[customer.customerType].label} color={colors.muted} name={SALES_CUSTOMER_TYPE_ICONS[customer.customerType] as never} size={14} /><Text style={styles.cardDescription}>{SALES_CUSTOMER_TYPE_PROFILES[customer.customerType].label}</Text></View></View><MaterialCommunityIcons color={colors.muted} name={isExpanded ? 'chevron-up' : 'chevron-down'} size={22} /></View><CustomerStats customer={customer} isExpanded={isExpanded} relationship={relationship} rejected={rejected} state={state} />{isExpanded && <RelationshipExplanation details={relationshipDetails} relationship={relationship} />}</Card.Content></Pressable></Card>; })}
  </>;
}
...
function Metric({ color = colors.muted, icon, label, suffix, value }: { color?: string; icon: string; label: string; suffix?: string; value: string }) {
  return <View style={localStyles.metric}><MaterialCommunityIcons accessibilityLabel={label} color={colors.muted} name={icon as never} size={14} /><Text style={styles.salesAvailability}>{`${label}: `}<Text style={{ color }}>{value}</Text>{suffix && <Text>{` · ${suffix}`}</Text>}</Text></View>;
}
...
function FacilityConditionReference() {
  return <Card mode="contained" style={styles.featureCard}><Card.Content style={localStyles.conditionReferenceContent}>
    ...
    <Text style={localStyles.formula}>Staff efficiency: 0.01 + 0.99 × ratio^1.6 when understaffed; 1 + 0.25 × (1 − e^(−0.7 × (ratio − 1))) when overstaffed. The ratio uses available workers and is multiplied by wage efficiency and Staff Quality.</Text>
...
const localStyles = StyleSheet.create({
  ...
  highlightedCustomer: { borderColor: colors.primary, borderWidth: 2 },
