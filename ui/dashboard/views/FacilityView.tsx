...
import { calculateOutputQuality, calculateProductionMaxQ, calculateUpgradeMaxQ, type OutputQualityBreakdown } from '@/game/quality';
...
function FacilityResourceSummary({ getQualityBreakdown, inventory, outputMultiplier, recipe, upgradeMaxQ }: { getQualityBreakdown: (resourceType: ResourceType, weightedInputQ: number | null, upgradeMaxQ: number) => OutputQualityBreakdown; inventory: Inventory; outputMultiplier: number; recipe: Recipe; upgradeMaxQ: number }) {
  const weightedInputQ = calculateRecipeInputQ(recipe, inventory);
  const inputMaxQ = weightedInputQ === null ? null : weightedInputQ + 1;
  ...
  const inputQualitySummary = inputMaxQ === null ? 'No inputs' : `${inputQualityDetails} → Q${formatNumber(inputMaxQ, { decimals: 2, forceDecimals: true })}`;
  return <View style={styles.facilityResourceSummary}>
    ...
    <View style={styles.facilityResourceGroup}><Text style={styles.facilityResourceLabel}>Output</Text><View style={styles.facilityResourceItems}>{recipe.outputs.map((output) => { const breakdown = getQualityBreakdown(output.resourceType, weightedInputQ, upgradeMaxQ); return <Text key={output.resourceType} accessibilityLabel={`${getResource(output.resourceType).name} ${formatNumber(output.amount * outputMultiplier, { smartDecimals: true })} at quality ${formatNumber(breakdown.outputQ, { decimals: 2, forceDecimals: true })}`} style={[styles.facilityResourceValue, styles.facilityResourceOutput]}>{getResource(output.resourceType).icon} {formatNumber(output.amount * outputMultiplier, { smartDecimals: true })} · Q{formatNumber(breakdown.outputQ, { decimals: 2, forceDecimals: true })}</Text>; })}</View></View>
    <View accessibilityLabel={`Quality calculation: input max ${inputMaxQ === null ? 'none' : `Q${formatNumber(inputMaxQ, { decimals: 2, forceDecimals: true })}`}; facility max Q${formatNumber(upgradeMaxQ, { decimals: 2, forceDecimals: true })}`} style={styles.facilityResourceQuality}><Text style={styles.facilityResourceQualityLabel}>Quality:</Text><Text style={styles.facilityResourceQualityText}>{inputQualitySummary} · </Text><MaterialCommunityIcons color={colors.muted} name={APP_ICONS.research} size={12} /><Text style={styles.facilityResourceQualityText}>{recipe.outputs.map((output) => `max Q${formatNumber(getQualityBreakdown(output.resourceType, weightedInputQ, upgradeMaxQ).researchMaxQ, { decimals: 2, forceDecimals: true })}`).join(', ')} · </Text><MaterialCommunityIcons color={colors.muted} name={APP_ICONS.upgrade} size={12} /><Text style={styles.facilityResourceQualityText}>{`max Q${formatNumber(upgradeMaxQ, { decimals: 2, forceDecimals: true })} · `}</Text><MaterialCommunityIcons color={colors.muted} name={APP_ICONS.output} size={12} /><Text style={styles.facilityResourceQualityText}>{recipe.outputs.map((output) => `max Q${formatNumber(getQualityBreakdown(output.resourceType, weightedInputQ, upgradeMaxQ).productionMaxQ, { decimals: 2, forceDecimals: true })}`).join(', ')}</Text></View>
  </View>;
}
