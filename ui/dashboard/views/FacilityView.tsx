...
export function ProductionView({
  ...
  return <>
    ...
    {orderedFacilities.map(({ facility, group }, index) => {
      ...
      const { assignedWorkers, conditionDecayMultiplier, conditionDecayUpgradeLevel, facilityEfficiency, facilityCondition, outputMultiplier, outputUpgradeLevel, overstaffingConditionDecayMultiplier, staffQuality, staffQualityTrend, pendingRepair, pendingStaffingChange, upgradeMaxQ, qualityUpgradeLevel, requiredWorkers, speedUpgradeLevel, speedUpgradeWorkSpeedMultiplier } = facilityView;
      ...
      const decayCostPerMinute = calculateFacilityDecayCostPerMinute(definition.landCost, definition.constructionMaterialsCost, definition.industrialMachinesCost, facilityCondition, totalConditionDecayMultiplier, effectiveWorkPerMinute, activeRecipe ?? null, market.getLocalPrice(ResourceType.ConstructionMaterials), market.getLocalPrice(ResourceType.IndustrialMachines));
      ...
      const qualityInputQ = activeRecipe ? facilityView.recipeInputQ ?? calculateRecipeInputQ(activeRecipe, inventory) : null;
      const getOutputQuality = (recipe: Recipe, weightedInputQ: number | null, resourceType: ResourceType): number => calculateOutputQuality({
        researchMaxQ: getResourceResearchMaxQ(resourceType, completedResearchProjectIds),
        weightedInputQ,
        upgradeMaxQ,
        productionMaxQ: calculateProductionMaxQ(producedByResource[resourceType]),
        staffMaxQ: facilityView.staffQuality,
      }).outputQ;
      const getActiveOutputQuality = activeRecipe ? (resourceType: ResourceType) => getOutputQuality(activeRecipe, qualityInputQ, resourceType) : undefined;
      const qualityUpgradeActualGain = activeRecipe ? Math.max(0, ...activeRecipe.outputs.map((output) => calculateOutputQuality({ researchMaxQ: getResourceResearchMaxQ(output.resourceType, completedResearchProjectIds), weightedInputQ: qualityInputQ, upgradeMaxQ: calculateUpgradeMaxQ(qualityUpgradeLevel + 1), productionMaxQ: calculateProductionMaxQ(producedByResource[output.resourceType]), staffMaxQ: facilityView.staffQuality }).outputQ - calculateOutputQuality({ researchMaxQ: getResourceResearchMaxQ(output.resourceType, completedResearchProjectIds), weightedInputQ: qualityInputQ, upgradeMaxQ, productionMaxQ: calculateProductionMaxQ(producedByResource[output.resourceType]), staffMaxQ: facilityView.staffQuality }).outputQ)) : 0;
      ...
      return <View key={facilityId}>{showGroup && <Text style={styles.cardKicker}>{group.label}</Text>}<Card mode="contained" style={[styles.featureCard, isFirstFacility && !focusTarget && styles.tutorialFirstFacilityCard]}><Card.Content>
        ...
        {!isExpanded && activeRecipe && <FacilityProductionStatus compact decayCostPerMinute={decayCostPerMinute} effectiveWorkPerMinute={effectiveWorkPerMinute} getInputQuality={(resourceType) => inventory.getQuality(resourceType)} getOutputQuality={getActiveOutputQuality} market={market} outputMultiplier={outputMultiplier} progress={facilityView.recipeProgress[activeRecipe.name] ?? 0} recipe={activeRecipe} staffWagePerMinute={calculateFacilityStaffWagePerMinute(assignedWorkers, facilityView.staffWagePerWorkerPerMinute)} status={productionStatus} />}
        {isExpanded && <>
          <View style={styles.facilityProductionSection}>
            {activeRecipe && <View style={styles.facilityProductionTop}><FacilityResourceSummary inputQ={qualityInputQ} inputSourceCost={facilityView.recipeInputSourceCost} maintenanceCost={calculateFacilityProductionMaintenanceCost(facilityView, activeRecipe, market)} staffMaxQ={facilityView.staffQuality} getQualityBreakdown={(resourceType, weightedInputQ, facilityMaxQ) => calculateOutputQuality({ researchMaxQ: getResourceResearchMaxQ(resourceType, completedResearchProjectIds), weightedInputQ, upgradeMaxQ: facilityMaxQ, productionMaxQ: calculateProductionMaxQ(producedByResource[resourceType]), staffMaxQ: facilityView.staffQuality })} inventory={inventory} outputMultiplier={outputMultiplier} recipe={activeRecipe} upgradeMaxQ={upgradeMaxQ} /><View style={styles.facilityRecipeActions}>
            ...
            <FacilityProductionStatus decayCostPerMinute={decayCostPerMinute} effectiveWorkPerMinute={effectiveWorkPerMinute} getInputQuality={(resourceType) => inventory.getQuality(resourceType)} getOutputQuality={getActiveOutputQuality} market={market} outputMultiplier={outputMultiplier} progress={activeRecipe ? facilityView.recipeProgress[activeRecipe.name] ?? 0 : 0} recipe={activeRecipe ?? null} staffWagePerMinute={calculateFacilityStaffWagePerMinute(assignedWorkers, facilityView.staffWagePerWorkerPerMinute)} status={productionStatus} />
          </View>
          ...
          {activeDetailTab === 'efficiency' && <View style={[styles.facilityEfficiencySection, isFirstFacility && firstFacilityStep === 'efficiency' && styles.tutorialFirstFacilityHeader]}>
            ...
            <View style={styles.facilityEfficiencyControls}>
              ...
              <View style={styles.facilityUpgradeAction}><Text style={styles.facilityStaffingDetail}>Staff {formatNumber(assignedWorkers)} / {formatNumber(requiredWorkers)}</Text><View style={{ flexDirection: 'row' }}><IconButton accessibilityLabel={`Open staffing controls for ${facilityName}`} icon={APP_ICONS.staffing} mode="contained" onPress={() => setStaffWageFacilityId(facilityId)} size={16} /><IconButton accessibilityLabel={`${facilityView.staffTraining ? 'Add staff to' : 'Open'} training for ${facilityName}`} icon={APP_ICONS.training} mode="contained" onPress={() => setStaffTrainingFacilityId(facilityId)} size={16} /></View></View>
                <View style={styles.facilityEfficiencyRow}><Text style={styles.facilityEfficiencyLabel}>Staff Quality</Text><Text style={[styles.facilityEfficiencyValue, { color: getColorClass(Math.min(1, staffQuality / 100)) }]}>Q{formatNumber(staffQuality, { decimals: 2, forceDecimals: true })} {staffQualityTrend === 'rising' ? '↑' : staffQualityTrend === 'falling' ? '↓' : '→'}</Text></View>
          ...
          {activeDetailTab === 'finance' && <View style={styles.facilityUpgradesSection}>
            ...
            <View style={styles.facilityFinanceSection}>
              ...
              <View style={styles.facilityFinanceRows}>
                ...
                <View style={styles.facilityEfficiencyRow}><Text style={styles.facilityEfficiencyLabel}>Output source cost</Text><Text style={styles.facilityEfficiencyValue}>-{formatCurrency(facilityPerformance.sourceCost)}</Text></View>
                ...
                <View style={styles.facilityEfficiencyRow}><Text style={styles.facilityEfficiencyLabel}>Hiring & severance</Text><Text style={styles.facilityEfficiencyValue}>-{formatCurrency(facilityPerformance.staffingExpense)}</Text></View>
            ...
            <Text style={styles.facilityRepairCost}>Operating profit is output value less direct input cost, repairs, and staff wages in the selected period. Capital investment is shown separately in the investment-adjusted result.</Text>
          </View>}
        </>}
      </Card.Content></Card><FacilityRepairDialog activeRecipe={activeRecipe ?? null} autoRepairLimit={getFacilityAutoRepairLimit(completedResearchProjectIds)} currentGameTimeMs={currentGameTimeMs} facility={facility} finance={finance} getInputQuality={(resourceType) => inventory.getQuality(resourceType)} getOutputQuality={getActiveOutputQuality} inventory={inventory} market={market} onDismiss={() => setRepairFacilityId(null)} onRepair={(targetCondition) => { const started = repairFacility(facilityId, targetCondition); if (started) setRepairFacilityId(null); return started; }} onSetAutoRepair={(enabled, threshold, target) => setFacilityAutoRepair(facilityId, enabled, threshold, target)} recipeResearchWorkSpeedMultiplier={activeRecipe ? getRecipeResearchWorkSpeedMultiplier(activeRecipe.name, completedResearchProjectIds) : 1} visible={repairFacilityId === facilityId} /><FacilityStaffWageDialog activeRecipe={activeRecipe ?? null} currentGameTimeMs={currentGameTimeMs} facility={facility} getInputQuality={(resourceType) => inventory.getQuality(resourceType)} getOutputQuality={getActiveOutputQuality} market={market} onDismiss={() => setStaffWageFacilityId(null)} onSetStaffWage={(wagePerWorkerPerMinute) => setFacilityStaffWage(facilityId, wagePerWorkerPerMinute)} onSetWorkers={(workerCount) => setFacilityWorkers(facilityId, workerCount)} recipeResearchWorkSpeedMultiplier={activeRecipe ? getRecipeResearchWorkSpeedMultiplier(activeRecipe.name, completedResearchProjectIds) : 1} visible={staffWageFacilityId === facilityId} /><FacilityStaffTrainingDialog currentGameTimeMs={currentGameTimeMs} facility={facility} onDismiss={() => setStaffTrainingFacilityId(null)} onTrainStaff={(workerCount) => trainFacilityStaff?.(facilityId, workerCount) ?? false} visible={staffTrainingFacilityId === facilityId} /></View>;
    })}
...
function RecipeOption({ canResearch, decayCostPerMinute, effectiveWorkPerMinute, freeTutorialResearch, getOutputQuality, inventory, locked, market, onPress, onResearch, outputMultiplier, recipe, selected, staffWagePerMinute }: { canResearch: boolean; decayCostPerMinute: number; effectiveWorkPerMinute: number; freeTutorialResearch: boolean; getOutputQuality: (resourceType: ResourceType) => number; inventory: Inventory; locked: boolean; market: Market; onPress: () => void; onResearch: () => void; outputMultiplier: number; recipe: Recipe; selected: boolean; staffWagePerMinute: number }) {
  ...
  const valuePerMinute = calculateRecipeValuePerMinute(recipe, market, outputMultiplier, effectiveWorkPerMinute, (resourceType) => inventory.getQuality(resourceType), getOutputQuality);
...
function FacilityResourceSummary({ getQualityBreakdown, inputQ, inputSourceCost, inventory, maintenanceCost, outputMultiplier, recipe, staffMaxQ, upgradeMaxQ }: { getQualityBreakdown: (resourceType: ResourceType, weightedInputQ: number | null, upgradeMaxQ: number) => OutputQualityBreakdown; inputQ: number | null; inputSourceCost: number | null; inventory: Inventory; maintenanceCost: number; outputMultiplier: number; recipe: Recipe; staffMaxQ: number; upgradeMaxQ: number }) {
...
function FacilityProductionStatus({ compact = false, decayCostPerMinute, effectiveWorkPerMinute, getInputQuality, getOutputQuality, market, outputMultiplier, progress, recipe, staffWagePerMinute, status }: { compact?: boolean; decayCostPerMinute: number; effectiveWorkPerMinute: number; getInputQuality: (resourceType: ResourceType) => number; getOutputQuality?: (resourceType: ResourceType) => number; market: Market; outputMultiplier: number; progress: number; recipe: Recipe | null; staffWagePerMinute: number; status: 'not-started' | 'paused' | 'missing-inputs' | 'producing' }) {
  ...
  const valuePerMinute = calculateRecipeValuePerMinute(recipe, market, outputMultiplier, effectiveWorkPerMinute, getInputQuality, getOutputQuality);
  const netGainPerMinute = calculateFacilityNetGainPerMinute(valuePerMinute, decayCostPerMinute, market, staffWagePerMinute);
