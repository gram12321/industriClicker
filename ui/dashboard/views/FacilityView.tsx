...
function FacilityResourceSummary({ getQualityBreakdown, inputQ, inputSourceCost, inventory, maintenanceCost, outputMultiplier, recipe, staffMaxQ, upgradeMaxQ }: { getQualityBreakdown: (output: RecipeOutput, weightedInputQ: number | null, upgradeMaxQ: number) => OutputQualityBreakdown; inputQ: number | null; inputSourceCost: number | null; inventory: Inventory; maintenanceCost: number; outputMultiplier: number; recipe: Recipe; staffMaxQ: number; upgradeMaxQ: number }) {
  ...
  return <View style={styles.facilityResourceSummary}>
    ...
    <View accessibilityLabel={`Quality limits: ${inputQualitySummary || 'no inputs'} to ${outputQualitySummary || 'no outputs'}; research ${researchQualitySummary || 'none'}; upgrade Q${qualityNumber(upgradeMaxQ)}; production ${productionQualitySummary || 'none'}; staff Q${qualityNumber(staffMaxQ)}${outputBonusSummary ? `; ${outputBonusSummary}` : ''}`} style={styles.facilityResourceQuality}>
      ...
      {outputBonusSummary && <><Text style={styles.facilityResourceQualityText}>+</Text><Text style={styles.facilityResourceQualityText}>{outputBonusSummary}</Text></>}
    </View>
  </View>;
}
