...
export function InventoryView({ buyMarketResource, currentGameTimeMs, facilities, finance, inventory, market, onlyInStock, resourceFlow, showActiveRecipeInputs, sellMarketResource, setMarketAutomation, setOnlyInStock, setShowActiveRecipeInputs }: {
  ...
  return <>
    ...
    {selectedResource && <View accessibilityLabel="Inventory flow period" style={localStyles.flowPeriodPicker}>{INVENTORY_FLOW_PERIODS.map((period) => <Button compact key={period.id} mode={period.id === flowPeriod.id ? 'contained' : 'outlined'} onPress={() => setFlowPeriodId(period.id)}>{period.label}</Button>)}</View>}
...
const localStyles = StyleSheet.create({
  ...
  flowPeriodPicker: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 10 },
  groupHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' },
  resourceFlow: { backgroundColor: colors.paleGreen, borderRadius: 10, gap: 6, padding: 10 },
  resourceFlowHeader: { color: colors.muted, fontSize: 11, fontWeight: '800', textTransform: 'uppercase' },
  resourceFlowNet: { borderTopColor: colors.muted, borderTopWidth: StyleSheet.hairlineWidth, marginTop: 2, paddingTop: 6 },
  resourceFlowRow: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' },
  resourceFlowValue: { fontWeight: '800' },
});

function MarketCard({ buyMarketResource, currentGameTimeMs, finance, flowPeriod, inventory, market, multiplier, openSettings, resourceFlow, resourceType, sellMarketResource, setMarketAutomation }: {
  buyMarketResource: (resourceType: (typeof RESOURCE_TYPES)[number], amount: number) => boolean;
  currentGameTimeMs: number;
  finance: Finance;
  ...
  const localSupplyTrend = localRegionalDiffusion.direction === 'to-local' ? 'up' : localRegionalDiffusion.direction === 'to-regional' ? 'down' : 'none';
  const globalSupplyTrend = regionalGlobalDiffusion.direction === 'to-global' ? 'up' : regionalGlobalDiffusion.direction === 'to-regional' ? 'down' : 'none';
...
function ResourceFlowCard({ currentGameTimeMs, flowPeriod, resourceFlow, resourceType }: { currentGameTimeMs: number; flowPeriod: (typeof INVENTORY_FLOW_PERIODS)[number]; resourceFlow: ResourceFlowLedger; resourceType: (typeof RESOURCE_TYPES)[number] }) {
